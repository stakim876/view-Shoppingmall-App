    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      ...
      >
        ...
        <div class="px-4 pb-4 flex gap-2">
          <router-link
            :to="`/product/${item.product_id}`"
            class="flex-1 py-2 text-center rounded-lg bg-indigo-500 text-white hover:bg-indigo-600 transition"
          >
            자세히 보기
          </router-link>
          <button
            @click.prevent="removeItem(item.product_id)"
            class="px-4 py-2 rounded-lg border border-rose-300 text-rose-600 dark:text-rose-400
