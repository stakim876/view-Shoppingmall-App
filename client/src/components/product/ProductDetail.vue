              ...
              <button
                ...
                :class="[
                  ...
                  stockNumber === 0
                    ...
                    : 'bg-gradient-to-r from-blue-600 to-indigo-500 text-white hover:from-indigo-600 hover:to-purple-500'
                ]"
              >
                장바구니 담기
              </button>
      ...
      <section class="max-w-4xl mx-auto px-6 py-12 border-b border-neutral-200">
        ...
        <form v-if="auth.isLoggedIn" @submit.prevent="submitReview" class="mb-8 p-4 bg-neutral-50 rounded-xl">
          ...
          <button
            ...
            class="px-4 py-2 bg-indigo-500 text-white text-sm rounded-lg hover:bg-indigo-600 disabled:opacity-50"
          >
...
const productImages = computed(() => {
  ...
  const list = product.value.images || (product.value.image_url ? [product.value.image_url] : []);
  const arr = Array.isArray(list) ? list : [product.value.image_url];
  return arr.map((url) => (url ? normalizeImageUrl(url) : "")).filter(Boolean);
});
